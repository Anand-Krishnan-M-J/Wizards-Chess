...
export const SideDrawer = ({
  ...
  useEffect(() => {
    if (isConnectionEstablished) {
      router.push(
        `/game?gameId=${!!callId ? callId : callIdRef.current?.value}&whoAmiI=${
  ...
  const tabs = [
        ...
        <>
          ...
          {callId && !isConnectionEstablished && (
            <div className={styles.drawer__generate__code__container}>
              ...
              <div
                className={styles.drawer__generate__code__wrapper}
                onClick={() => {
                  copy(callId);
                  setIsCodeCopied(true);
                }}
              >
                <span className={styles.drawer__generate__code}>{callId}</span>
                <Image
                  src={copyImg}
                  alt="close"
                  width={30}
              ...
              {!isConnectionEstablished && (
                <div className={styles.drawer__establish__conn__wrapper}>
                  <Loader />
                  <p className={styles.drawer__establish__conn}>
    ...
    },
    {
      ...
      content: (
        <>
          <CustomInputField
            error={error.gameCode ? messages.gameCodeEmptyError : ''}
          ...
          />
          {!isConnectionEstablished && (
            <CustomButton onClick={handleConnect} className={styles.drawer__generate__code__button}>
    ...
    <div ref={drawerRef} className={`${styles['side-drawer']} ${isDrawerOpen ? styles.open : ''}`}>
      ...
      <div className={styles.drawer__content}>
        ...
        <div className={styles.drawer__sub__content}>
          ...
          <div className={styles.drawer__generate__code__container}>
            {isConnectionEstablished && (
              <div className={styles.drawer__conn__established__wrapper}>
            ...
            <div className={styles.drawer__video__outer}>
              ...
              {!shouldHideVideo && (
                <div className={styles.drawer__video__container}>
                  <div className={enableVideoDrawer && isGamePage ? styles.video__drawer__you : ''}>
                    ...
                    <video
                      ...
                      ref={isSharing ? localVideoRef : null}
                      autoPlay
                      playsInline
                      muted
                    />
                    <p className={styles.drawer__video__text}>
                      {myName}
                      {isSharing && (
                        <span>
                          <button className={styles.video__disable} onClick={() => stopSharing()}>
                            <VideoDisable />
                          </button>
                        </span>
                      )}
                  ...
                  <div
                    className={
                      enableVideoDrawer && isGamePage ? styles.video__drawer__opponent : ''
                    }
                  >
                    {enableVideoDrawer && <CustomEndLine isSmall />}
                    <video
