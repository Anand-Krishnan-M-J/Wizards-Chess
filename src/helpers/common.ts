...
export const isSquareOccupied = (col: ColName, row: RowName, pieces: PieceState[]) => {
  return (
    pieces.find((item) => item.currentCol === col && item.currentRow === row && !item.kia) !==
    undefined
  );
};
  ...
  return (
    pieces.find(
      (item) =>
        ...
        item.currentRow === row &&
        !item.kia &&
        (enemyType === pieceTypeColor.white ? isWhitePiece(item.name) : !isWhitePiece(item.name))
    ) !== undefined
  );
};
export const isSquareOccupiedByKing = (col: ColName, row: RowName, pieces: PieceState[]) => {
  return (
    pieces.find(
      (item) => item.currentCol === col && item.currentRow === row && item.type === pieceTypes.king // Check if the piece is a king
    ) !== undefined
  );
};
