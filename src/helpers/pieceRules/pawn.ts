...
const getAttackablePositions = (name: pieceName, currentCol: ColName, currentRow: RowName) => {
...
export const getPawnMoves = (
  ...
  if (isWhitePiece(name)) {
    ...
    // if on any other row, can move only 1 step ahead
      ...
      if (!isSquareOccupied(currentCol, getNextRowName(currentRow, true), pieces)) {
        allowedMoves.push({
          col: currentCol,
  ...
  if (!isWhitePiece(name)) {
    ...
    // For all other rows
      ...
      // Forward move is disabled when a pawn is at front
      if (!isSquareOccupied(currentCol, getNextRowName(currentRow, false), pieces)) {
        allowedMoves.push({
          col: currentCol,
  ...
  const attackablePositions = getAttackablePositions(name, currentCol, currentRow)
    .filter(
      (pos) =>
        pieces.find((piece) => piece.currentCol === pos.col && piece.currentRow === pos.row)
          ?.type !== pieceTypes.king
    )
    .map((item) => `${item.col}${item.row}`);

  const pieceColor = isWhitePiece(name) ? pieceTypeColor.black : pieceTypeColor.white;

  const attackablePositionOccupiedByEnemy = allEnemies(pieceColor, pieces)
    .filter((item) => attackablePositions.includes(`${item.currentCol}${item.currentRow}`))
    .map((item) => ({
      col: item.currentCol,
      row: item.currentRow,
    }));
