  ...
  reducers: {
    ...
    selectPiece: (state: PieceReduxState, action) => {
      ...
      if (action.payload.name != null) {
        const { attackablePositionOccupiedByEnemy, allowedMoves } = getAllowedMoves(
    ...
    movePiece: (state: PieceReduxState, action) => {
      ...
      const pieceToKill = state.pieces.find(
        (piece) => piece.currentCol === col && piece.currentRow === row && !piece.kia
      ) as PieceState;
      ...
      // Move the piece
      const pieceToMove = state.pieces.find((item) => item.name === name) as PieceState;

      // Handle castling move
      if (pieceToMove.type === pieceTypes.king && pieceToMove.isCastlingAllowed) {
      ...
      if (pieceToMove.type === pieceTypes.rook) {
        ...
        const king = state.pieces.find(
          (piece) => piece.currentRow === pieceToMove.currentRow && piece.type === pieceTypes.king
        );
